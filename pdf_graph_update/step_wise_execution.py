...
*Weight conditioning here trials an efficient, unidirectional weight matrix (not used in time_based_execution).
"""
...
def condition_weight_matrix(G, mu=1.0, decay_parameter=2):
    ...
    distances = squareform(pdist(G))

    # Generate random values for all pairs
    ...
    edges_exist = (random_values < np.exp(-decay_parameter * distances)) & mask

    # Calculate the weights where edges exist
    weights = np.where(edges_exist, 1 / (1 + mu * distances), 0)
...
def check_reward(A, y_train):
    ...
    def calculate_reward(current_output, correct_class, std_dev=100, max_reward=100):
...
"""
Define a function for propagating information around the network using the activation/state vector A (N_total, 1)

    Args:
    A (np.array): Initial state vector of shape (N, 1)
    b (np.array): Bias vector of shape (N, 1)
    W (np.array): Weight matrix of shape (N, N)
    max_iterations (int): Maximum number of iterations to prevent infinite loops

    Returns:
    A (np.array): Updated state vector A (N,1)
    
*Propagation here is designed for stepwise execution. The steps are not conditioned on time intervals.
*Leaky RelU is used because propagation is unidirectional and has strict stopping conditions 
(unlike in time_based_execution).
"""

def propagate(A, b, W, y_train, max_iterations=10000):
    global image_counter
    current_output = A[-1, 0]

    iteration = 0
    while iteration < max_iterations:
        # Apply Leaky ReLU to biased states
        A = np.maximum(0.001* A + b, A + b)

        # Linearly combine the weighted states
        if sparse.issparse(W):
            A = W.dot(A)  # Use sparse matrix multiplication
        else:
            A = np.dot(W, A)  # Use regular matrix multiplication if W is dense
    ...
    print(f"\n### PROPAGATION - IMAGE {image_counter}:\n"
    ...
    if iteration >= max_iterations:
        print(f"Propagation stopped. Termination criterion reached. Iteration: {iteration}")
...
def realisation(f_coord, I_coord, O_coord, max_rows=200, sampling_radius=3):
    ...
    # noise = np.random.uniform(-0.0000;pl, 0.0000, size=f_current.shape)  # Add small uniform noise
...
# step-wise execution
def step_wise_execution(x_train, y_train, I_coord, O_coord, f_coord, G, A, b, W, num_steps=60000):
    for step in range(num_steps):
        print(f"\n--- Step {step + 1} ---")

        # Propagate
        A = propagate(A, b, W, y_train, max_iterations=10000)
        ...
        # Realise updated Graph and activations
...
#   print(f"b shape: {b.shape}, W shape: {W.shape}, A shape: {A.shape}, G shape: {G.shape}")
    step_wise_execution(x_train, y_train, I_coord, O_coord, f_coord, G, A, b, W, num_steps=60000)
    show_pdf_and_nodes(f_coord, G)
