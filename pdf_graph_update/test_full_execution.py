...
Define a function for loading and preparing the MNIST dataset
     ...
     Returns: (x_train, y_train), (x_test, y_test) (tuples)
"""
...
The plane is separated along the x-axis by a distance specified by the 'x' parameter.
...
Define a function for intialising the network at t=0 within the unit cube
    ...
    resolution (float): The number of points between the upper and lower bound of the grid
...
def check_reward(A, y_train):
    ...
    def calculate_reward(current_output, correct_class, std_dev=1, max_reward=100):
...
"""
Define a Propagator class to manage continual activation state propagation in a separate thread to the update.

Propagator applies sigmoid activation thresholding to update node states (to +1 or -1),
These states propagate through the network using the weight matrix.

The class contains methods to start, stop, and update the propagation process, and retrieve current activation states.
A threading mechanisms supports thread-safe operations a for concurrent execution with the update processes.
Threading is useful because concurrent executions can disrupt the timing of "time_based_execution"
...
def update_PDF_scalar_field(f_coord, G, current_reward, m=None):
    ...
    def generate_summed_gaussians(f_coord, G, m):
        ...
        for i, node in enumerate(G):
            ...
            # Some neurons are more impotant than others.
    ...
    m_sum = np.sum(m)  # Compute normalization factor
    if m_sum == 0:
    ...
    # Generate random scaling factor (alternate: alpha = np.random.normal(0.5, 1))
    alpha = 1
    f_current = f_coord[:, :, :, 3]

    # Update field. When the current reward is large and positive, amplify the PDF around the node values (f+reward*phi)
...
def realisation(f_coord, I_coord, O_coord, max_rows=400):
    # Extract the scalar field values (4th dimension).
    # 'f_coord' is a (res, res, res, 4) array containing res^3 (x,y,z,f(.)) values
    f_current = f_coord[:, :, :, 3]

    # Add random noise so the amplified values don't dominate in subsequent realisation stages.
    noise = np.random.uniform(-0.0005, 0.0005, size=f_current.shape)  # Add small uniform noise
    f_current = f_current + noise

    # Use maximum filter to find local maxima. Iterate over the 4th value in each (x,y,z,f(.)) element.
    # For every f(.) check the neighbouring f(.) values in a 3 x 3 cube within the array.
    local_max = ndimage.maximum_filter(f_current, size=3)

    # Create a mask where True indicates that the original value equals the local maximum
    ...
    G_medial = f_coord[mask][:, :3]
    # Get indices of maxima
    # Extract the corresponding (x, y, z) coordinates
    if len(G_medial) > max_rows:
        excess_rows = len(G_medial) - max_rows
        drop_indices = np.random.choice(len(G_medial), excess_rows, replace=False)
        G_medial = np.delete(G_medial, drop_indices, axis=0)

    G_new, A = combine_fullgraph_and_states(I_coord, G_medial, O_coord)

    print(f"\n### REALISATION - IMAGE {image_counter}:\n"
...
""" 
Define a function for displaying the graph (viewed down from above the XY plane). Also displays an XY 'slice' of the PDF.
